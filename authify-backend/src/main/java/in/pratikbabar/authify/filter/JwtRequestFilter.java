package in.pratikbabar.authify.filter;
...
public class JwtRequestFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        //no need to authenticate these urls== PUBLIC_URLS
        ...
        String jwt =null;
        String email = null;
        //1. check the authorization header if not using cookies and token is comming with every request
        final String authorizationHeader = request.getHeader("Authorization");
        if(authorizationHeader!=null && authorizationHeader.startsWith("Bearer ")){
          jwt=  authorizationHeader.substring(7);
        }
        //2.If not found in authorization header then look for cookie
        if(jwt==null){
